...
```bash
python3 scripts/x_workflow.py --help
```
...
```bash
python3 scripts/x_workflow.py home view \
  --count 30

python3 scripts/x_workflow.py home download \
  --count 30
```
...
```bash
python3 scripts/x_workflow.py bookmarks view \
  --count 30

python3 scripts/x_workflow.py bookmarks download \
  --count 30
```
...
```bash
python3 scripts/x_workflow.py tweet view \
  --url "https://x.com/<handle>/status/<id>"

python3 scripts/x_workflow.py tweet download \
  --url "https://x.com/<handle>/status/<id>"
```
...
```bash
python3 scripts/x_workflow.py thread view \
  --url "https://x.com/<handle>/status/<id>" \
  --count 50

python3 scripts/x_workflow.py thread download \
  --url "https://x.com/<handle>/status/<id>" \
  --count 50
```
...
```bash
python3 scripts/x_workflow.py search view \
  --query "关键词" \
  --filter live \
  --count 30

python3 scripts/x_workflow.py search download \
  --query "关键词" \
  --filter live \
  --count 30
```
...
```bash
python3 scripts/x_workflow.py profile view \
  --handle "@handle" \
  --count 30

python3 scripts/x_workflow.py profile download \
  --handle "@handle" \
  --count 30
...
python3 scripts/x_workflow.py me view \
  --count 30

python3 scripts/x_workflow.py me download \
  --count 30
```
